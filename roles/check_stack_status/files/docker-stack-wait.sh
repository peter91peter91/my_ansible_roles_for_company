...
stack_done=0
service_not_ready="not set"
state_not_ready="not set"
while [ "$stack_done" != "1" ]; do
  ...
  for service_id in ${service_ids}; do
    ...
    task_status=$(docker service ps "$service" --format '{{.CurrentState}}' | cut -d' ' -f1 | awk '{print tolower($0)}' | head -1)
    task_time=$(docker service ps "$service" --format '{{.CurrentState}}' | cut -d' ' -f2 | awk '{print tolower($0)}' | head -1)
    task_unit_of_time=$(docker service ps "$service" --format '{{.CurrentState}}' | cut -d' ' -f3 | awk '{print tolower($0)}' | head -1)

    waiting_time="30"
    case $task_unit_of_time in
      minutes|hours)
        waiting_time=1;;
    esac
    ...
    if echo "$task_time" | grep -Eq '^[0-9]+$'; then
      if [ "$task_status" = 'complete' ] && [ "$task_time" -ge "$waiting_time" ]; then
        service_done=1
      fi;
    fi;
    ...
    if [ "$service_done" != "2" ]; then
      ...
      if [ "$current" != "$target" ] && [ "$task_status" != "complete" ]; then
        # actively replicating service
        service_done=0
        service_waiting="$service"
    ...
    if [ "$service_done" = "1" ]; then
      case "$state" in
        completed|complete|running|rollback_completed)
          service_done=1
          ;;
        failed)
          service_done=2
    ...
    if [ "$service_done" = "2" ]; then
      ...
      docker service logs "$service_id" --tail 100
      stack_done=2
...
#    echo "Print logs for all services"
#    for service_id in ${service_ids}; do
#      service=$(docker service inspect --format '{{.Spec.Name}}' "$service_id")
#      echo "Service logs for $service"
#      docker service logs "$service_id" --tail 50
#    done
    exit 1
  fi
  if [ "$stack_done" != "1" ]; then
    check_timeout
    echo "Service: ${service_waiting} State: ${state_not_ready}, wait ${opt_s}s..."
    sleep "${opt_s}"
  fi
done
